...
\begin{bullets}
\item \textit{Online Min-Cost Matching with General Arrivals} - accepted in \textit{ACM Conference on Economics and Computation} (EC '26)\\
with with Eric Balkanski, Jason Chatzitheodorou, and Vasilis Gkatzelis
\item \textit{An $O(\log n)$-Competitive Posted-Price Algorithm for OML} - published in \textit{Conference on Combinatorial Optimization and Applications} (COCOA '23)\\
with Stephen Arndt and Kirk Pruhs
\item \textit{Resource Augmentation Analysis of the Greedy Algorithm for the Online Transportation Problem} - published in  \textit{Latin American Algorithms, Graphs, and Optimization Symposium} (LAGOS '23)\\
with Stephen Arndt and Kirk Pruhs
